  ...
  return (
    <>
      ...
      <div className={`md:hidden fixed top-0 left-0 w-full h-full bg-black/50 backdrop-blur-sm z-40 transition-all duration-300 ease-out ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        <nav className={`flex flex-col items-center justify-center h-full space-y-8 transition-all duration-300 ease-out ${isOpen ? 'translate-y-0' : '-translate-y-8'}`}>
          {navItems.map((item, index) => (
            <a
              key={item.name}
              href={item.href}
              onClick={() => setIsOpen(false)}
              className={`text-white uppercase font-medium text-xl hover:text-pink-400 transition-all duration-300 hover:scale-105 transform ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
